...
class CBRecommend:
    ...
    def __init__(self, K):
        #给用户推荐的item个数
        self.K = K
        self.item_profile = json.load(
            open("datasets/MyFirstrecomendationer-movie/item_profile.json",
                 "r"))
        self.user_profile = json.load(
            open("datasets/MyFirstrecomendationer-movie/user_profile.json",
                 "r"))

    #获取用户未进行评分的item列表
    def get_none_score_item(self, user):
        items = pd.read_csv("datasets/MyFirstrecomendationer-movie/movies.csv",
                            encoding='gbk')["MovieID"].values
        data = pd.read_csv("datasets/MyFirstrecomendationer-movie/ratings.csv",
                           encoding='gbk')
        have_score_items = data[data["UserID"] == user]["MovieID"].values
        nonee_score_items = set(items) - set(have_score_items)
        return nonee_score_items
    ...
    def cosUI(self, user, item):
        ...
        Ua = math.sqrt(
            sum([math.pow(one, 2) for one in self.user_profile[str(user)]]))
        Ia = math.sqrt(
            sum([math.pow(one, 2) for one in self.item_profile[str(item)]]))
        return Uia / (Ua * Ia)
    ...
    def recommend(self, user):
        ...
        if self.K is None:
            result = sorted(user_result.items(),
                            key=lambda k: k[1],
                            reverse=True)
        else:
            result = sorted(user_result.items(),
    ...
    def evaluate(self):
        ...
        for user in random.sample([one for one in range(1, 6040)], 20):
            have_score_items = data[data["UserID"] == user]["MovieID"].values
            items = pd.read_csv(
                "datasets/MyFirstrecomendationer-movie/movies.csv",
                encoding='gbk')["MovieID"].values
            user_result = {}
            for item in items:
                user_result[item] = self.cosUI(user, item)
            results = sorted(user_result.items(),
                             key=lambda k: k[1],
