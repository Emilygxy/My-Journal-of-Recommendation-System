...
class ItemCFRec:
    ...
    def ItemSimilarityBest(self):
        print("开始计算物品之间的相似度......")
        if os.path.exists("datasets/RecomendationerBasedonItemCF-movie/item_sim.json"):
            print("物品相似度从文件加载......")
            itemSim = json.load(open("datasets/RecomendationerBasedonItemCF-movie/item_sim.json","r"))
        else:
            itemSim = dict()
            item_user_count = dict()#得到每个物品有多少个用户产生过行为
            count = dict()#同现矩阵
        ...
        json.dump(itemSim,open("datasets/RecomendationerBasedonItemCF-movie/item_sim.json","w"))
        return itemSim
